package org.foodiepass.server.domain;
...
public enum Currency {
    ...
    private static Map<String, Currency> BY_NAME;
    private static Map<String, Currency> BY_CODE;
    ...
    /**
     * 초기화된 맵을 반환하는 메서드
     * Lazy Initialization을 사용하여 처음 접근할 때 맵을 초기화
     */
    private static Map<String, Currency> getByNameMap() {
        if (BY_NAME == null) {
            initializeMaps();
        }
        return BY_NAME;
    ...
    private static Map<String, Currency> getByCodeMap() {
        if (BY_CODE == null) {
            initializeMaps();
        }
        return BY_CODE;
    ...
    /**
     * 맵을 초기화하는 synchronized 메서드로 동시성 문제 방지
     */
    private static synchronized void initializeMaps() {
        if (BY_NAME == null && BY_CODE == null) {
            BY_NAME = new HashMap<>();
            BY_CODE = new HashMap<>();
            for (Currency currency : values()) {
                // 대소문자 구분 없이 매핑되도록 미리 변환
    ...
    /**
     * 대소문자 구분 없이 화폐 이름으로 Currency를 반환하는 메서드
     */
    public static Currency fromCurrencyName(final String currencyName) {
        return getByNameMap().get(currencyName.toUpperCase());
    }

    /**
     * 대소문자 구분 없이 ISO 4217 코드로 Currency를 반환하는 메서드
     */
    public static Currency fromISO4217Code(final String code) {
        return getByCodeMap().get(code.toUpperCase());
    }
}
